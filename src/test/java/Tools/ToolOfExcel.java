...
import spiderJava.Utils.Comparator2Note;
import spiderJava.entity.excel.*;
...
public class ToolOfExcel {
    ...
    private static final String SourceUrl2 = "http://www.chinafilm.org.cn/Item/list" +
            ".asp?id=1619&areaid=104&C=0000000000";
    ...
    private static final Pattern P2 = Pattern.compile("([省市])([\\u4e00-\\u9fa5]{1,5}(区|自治区|县))");
    private static final Pattern P21 = Pattern.compile("([\\u4e00-\\u9fa5]{1,5}(区|自治区|县))");

    private static final String SourceUrl3 = "http://www.icbc.com.cn/icbc/%E5%AE%A2%E6%88%B7%E6%9C%8D%E5%8A%A1" +
            "/business_offices.htm";
    private static final String ProvinceAndUrlCSS3 = "#dumaScrollAreaId_89Area > div:nth-child(2) > table:nth-child" +
            "(1) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(1) > div:nth-child(1) > span:nth-child(1) > " +
            "a:nth-child(1)";
    ...
    private static final String SourceUrl5 = "http://www.kfc.com.cn/kfccda/storelist/";
    private static final String ProvinceCss5 = ".city_info > li";
    private static final String FinishFlag5 = ".city_info";
    private static final Pattern P = Pattern.compile("(\\d+)");

    private static final String SourceUrl6 = "https://www.starbucks.com.cn/stores/";
    private static final String FinishFlag6 = ".footer";
    ...
    void json2ExcelGB2260() throws Exception {
        ...
        for (int i = 0; i < 2020 - startYear + 1; i++) {
            ...
            ExcelUtil.exportExcel(folderPath2 + "res/" + curYear + "codeOfRegion.xlsx", codeOfRegionList,
                    CodeOfRegion.class);
...
//        输出结果并没有排序,不过排序的事,excel很擅长!
        ExcelUtil.exportExcel(folderPath1 + "行政区划类型.xlsx", typeOfRegionList, TypeOfRegion.class);
    }

    @Test
    void comparator2Note() throws Exception {
        List<Comparator2Note> comparator2NoteList = ExcelUtil.readXls(folderPath + "Comparator2Note.xlsx",
                Comparator2Note.class);
...
//          有八种元素定位方法: d, name, class name, tag name, link text,部分link text, xpath, css选择器
    ...
    void getRailStationList() throws Exception {
        ...
        Map<String, String> provinceAndUrl = getProvinceAndUrl(SourceUrl1, ProvinceAndUrlCSS1,
                ProvinceAndUrlClickCSS2, FinishFlag1, false);
//        boolean flag = false;
...
//                这个line依然是全局定位的
                String line =
                        webDriver.findElement(By.xpath("/html/body/div[2]/div[2]/div[2]/div[2]/div[1]/p[1]")).getText();
    ...
    void getCinema() throws Exception {
        ...
        Map<String, String> provinceAndUrl = getProvinceAndUrl(SourceUrl2, ProvinceAndUrlCSS2,
                ProvinceAndUrlClickCSS2, FinishFlag2, true);
        for (Map.Entry<String, String> e1 : provinceAndUrl.entrySet()) {
        ...
        ExcelUtil.exportExcel(folderPath1 + "OriginalCinemaList.xlsx", longAllCinemaList, CinemaNum.class);
    ...
    void getICBCStation() {
        ...
        Map<String, String> provinceAndUrl = getProvinceAndUrl(SourceUrl3, ProvinceAndUrlCSS3,
                ProvinceAndUrlClickCSS3, FinishFlag3, true);
        for (Map.Entry<String, String> e1 : provinceAndUrl.entrySet()) {
...
//                    Thread.sleep(3000L);
        ...
        ExcelUtil.exportExcel(folderPath1 + "CMBStaionNumList1.5.xlsx", allStationNumList, BankStaionNum.class);
    ...
    void Ask() {
        ...
        ((JavascriptExecutor) webDriver).executeScript(js2);
//        System.out.println( webDriver.findElement(By.xpath("/html/body/div[3]/div[1]/div/select[1]/option[2]"))
//        .getText());
        Select provSelect = new Select(webDriver.findElement(By.cssSelector("select[name='provSelect']")));
...
//            provSelect.selectByValue(String.valueOf(i));
    ...
    @Test
    void getKFCStationByJsoup() throws Exception {
        List<KFCStationNum> allKFCStationNum = new ArrayList<>(4096);
        List<String> provinceOrCity = new ArrayList<>(256);
        try {
            Document doc = Jsoup.connect(SourceUrl5).get();
            Elements line = doc.getElementsByClass("city_city");
            for (Element l : line) {
                String[] ss = l.text().split(" ");
                provinceOrCity.addAll(Arrays.asList(ss));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        for (String s : provinceOrCity) {
            String content = httpPostParam(s);
//           如果rowCount为0,意味着根本没有KFC在这里自然不用看了
            if (!content.contains("\"rowcount\":0")) {
                System.out.println(content);
                JSONObject jsonObject = JSON.parseObject(content);
                JSONArray jsonArray = jsonObject.getJSONArray("Table1");
                for (Object obj : jsonArray) {
                    JSONObject jObj = (JSONObject) obj;
                    String provinceName = jObj.getString("provinceName");
                    String cityName = jObj.getString("cityName");
                    String addressDetail = jObj.getString("addressDetail");
                    KFCStationNum kfcStationNum = new KFCStationNum();
                    kfcStationNum.setProvince(provinceName);
                    kfcStationNum.setCounty(cityName);
                    kfcStationNum.setStation(addressDetail);
                    allKFCStationNum.add(kfcStationNum);
                }
            }

        }
        ExcelUtil.exportExcel(folderPath2 + "KFCStationNum.xlsx", allKFCStationNum, KFCStationNum.class);

    }

    @Test
    void getStarBuckStation() throws Exception {
        List<StarBuckStationNum> allStarBuckStationNum = new ArrayList<>(5000);
        File jsonFile = ResourceUtils.getFile(folderPath1 + "StarBucks" + ".json");
        String jsonString = FileUtils.readFileToString(jsonFile);
        JSONObject jsonObject = JSON.parseObject(jsonString);
        JSONArray jsonArray = jsonObject.getJSONArray("data");
        for (Object obj : jsonArray) {
            JSONObject jObj = (JSONObject) obj;
            JSONObject jsonAddress = (JSONObject) jObj.get("address");
            String city = jsonAddress.getString("city");
            String streetAddressLine1 = jsonAddress.getString("streetAddressLine1");
            String streetAddressLine2 = jsonAddress.getString("streetAddressLine2");
            String streetAddressLine3 = jsonAddress.getString("streetAddressLine3");
            StarBuckStationNum starBuckStationNum = new StarBuckStationNum();
            starBuckStationNum.setCity(city);
            starBuckStationNum.setStreetAddressLine1(streetAddressLine1);
            starBuckStationNum.setStreetAddressLine2(streetAddressLine2);
            starBuckStationNum.setStreetAddressLine3(streetAddressLine3);
            allStarBuckStationNum.add(starBuckStationNum);
        }
        ExcelUtil.exportExcel(folderPath1 + "OriginalStarBuckStationNum.xlsx", allStarBuckStationNum,
                StarBuckStationNum.class);
    }

    @Test
    void FailGetStarBuckStation() throws Exception {
        WebDriver webDriver = getHomePage(SourceUrl6, FinishFlag6, false);
//        点击"未知地点"
        webDriver.findElement(By.cssSelector(".LocationFilterSwitch")).click();
//        选择"省份-城市-县区"列表
        List<WebElement> es0 = webDriver.findElements(By.cssSelector(".LocationFilterPanel > div.LocationFilterTabs >" +
                " button"));

//        点击"省份"
        es0.get(0).click();
//        选择省份列表
        List<WebElement> ws0 = webDriver.findElements(By.cssSelector(".LocationFilterGroupContent > button"));
//        点击"安徽"
        ws0.get(0).click();
...
//        es0.get(1).click();
        ...
        List<WebElement> ws1 = webDriver.findElements(By.cssSelector(".LocationFilterLocations > button "));

//        选择城市列表--北京
//        List<WebElement> ws1 =  webDriver.findElements(By.cssSelector(".LocationFilterGroupContent > button"));
//        点击"合肥"
        ws1.get(1).click();
...
//        es0.get(2).click();
//        选择县区列表
        List<WebElement> ws2 = webDriver.findElements(By.cssSelector(".LocationFilterLocations > button"));
//        点击"瑶海"
        ws2.get(1).click();

//        点击搜索,但是前端不响应,或者一直转圈圈
        webDriver.findElement(By.cssSelector("#city-filter-apply")).click();
    }

    /**
     * https://blog.csdn.net/weixin_43235209/article/details/104572134
     */
    @Test
    private String httpPostParam(String str) throws Exception {
        //创建HttpClient对象
        CloseableHttpClient httpClient = HttpClients.createDefault();

        //创建HttpPost对象，设置url访问地址
        HttpPost httpPost = new HttpPost("http://www.kfc.com.cn/kfccda/ashx/GetStoreList.ashx?op=cname");

        //与Get请求不一样，Post请求需要声明List集合，封装表单中的参数
        List<NameValuePair> params = new ArrayList<>();

        //将参数封装进表单中
        params.add(new BasicNameValuePair("cname", str));
        params.add(new BasicNameValuePair("pageSize", "500"));
...
//            左上角的位置
    ...
    /**
     * 这个没有用,因为display:none的修改不能先于display所在的隐藏标签本身的出现
     */
    private Set<String> getProvinceOrCity4KFC(String sourceUrl, String finishFlag) {
        WebDriver webDriver = getHomePage(sourceUrl, finishFlag, true);
        List<WebElement> table = webDriver.findElements(By.cssSelector(ProvinceCss5));
        Set<String> provinceOrCities = new HashSet<>(256);
        for (WebElement t : table) {
            String ss = t.findElement(By.cssSelector(".city_city > a")).getText();
            System.out.println(ss);
            t.findElement(By.cssSelector("div > a"));
            System.out.println(t.getText());
            provinceOrCities.add(t.getText());
        }
        return provinceOrCities;
    }

    private Map<String, String> getProvinceAndUrl(String sourceUrl, String css, String clickCss, String finishFlag,
                                                  Boolean isClick) {
        WebDriver webDriver = getHomePage(sourceUrl, finishFlag, false);
        if (isClick) {
//            WebElement popButtonParent = webDriver.findElement(By.xpath
//            ("/html/body/form/div[2]/table/tbody/tr/td[2]/div/table/tbody/tr/td[1]/div"));
...
//           WebElement popButton = webDriver.findElement( By.xpath("//*[@id=\"icbccity\"]"));
    ...
    private Map<String, String> getCitiesAndUrl(Map.Entry<String, String> e, String css, String finishFlag,
                                                Boolean isClick) {
        WebDriver webDriver = getFirefoxWebDriver();
...
//        }
    ...
    private Map<String, Integer> getCitiesAndCinemaNum(Map.Entry<String, String> e, String css, String finishFlag,
                                                       Boolean isClick) {
        WebDriver webDriver = getFirefoxWebDriver();
        System.out.println("======" + e.getKey() + "=====");
        webDriver.get(e.getValue());
...
//                String county = washRegionData(text);
                countyAnmCinemaNum.put(text, countyAnmCinemaNum.getOrDefault(text, 0) + 1);
            }
        }
        webDriver.close();
        return countyAnmCinemaNum;
    ...
    /**
     * @param text
     * @return 正则是没有办法应对现实世界的数据的!!除非某个人负责的文档,或者严格规范下的
     * 正则最多专注于我能匹配的就是正确的,不能匹配的才是生活的常态
     */
